...
class sydClass{
						...
						while($row=$this->result->fetch_array(MYSQLI_NUM)){ ?>
							<tr>
								...
								<td class="td-actions text-center">
									...
									<button type="button" title='Edit' class="btn btn-primary btn-xs <?php echo "btn_edit_".str_replace('dt_', '', $dtTble); ?>" value="<?php echo $row[0];?>">

														<i class="fa fa-pencil"></i></button>
									<button type="button" title='Remove' class="btn btn-danger btn-xs <?php echo "btn_remove_".str_replace('dt_', '', $dtTble); ?>" value="<?php echo $row[0];?>" style="margin-left: 2%;"><i class="fa fa-trash-o "></i></button>
	...
	public function fillCombo($qry,$id,$choose){
		$this->connection();
		echo"<div class='form-group has-success '> <select id='$id' class='form-control has-success selects fontawesome-select' name='$id' style='width:100%;' required=''>";
		try{
			$this->result=$this->db->query($qry);
			$condition=(bool)mysqli_num_rows($this->result);
			echo $condition;
